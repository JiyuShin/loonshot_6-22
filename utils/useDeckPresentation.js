...
export function useDeckPresentation(deckRef) {
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      // 2-3페이지 (SlideFord -> SlideAnnexHistory) 스크롤 진행도 계산
      const fordIndex = slides.findIndex((s) => s.classList.contains("figma-ford"));
      if (fordIndex !== -1 && slides[fordIndex + 1]) {
        const startTop = slides[fordIndex].offsetTop;
        const endTop = slides[fordIndex + 1].offsetTop;
        const distance = endTop - startTop;
        const fordProgress = distance > 0 ? Math.max(0, Math.min(1, (top - startTop) / distance)) : 0;
        
        // Slide 2 (SlideFord)에 진행도 설정
        slides[fordIndex].style.setProperty("--scroll-progress-ford", fordProgress.toFixed(3));
        
        // Slide 3 (SlideAnnexHistory)에도 동일한 진행도 설정
        slides[fordIndex + 1].style.setProperty("--scroll-progress-ford", fordProgress.toFixed(3));

        // 스크롤이 시작되면 경계면을 완벽한 흰색으로 채워 검은 선(틈새)이 보이지 않도록 함
        const edgeOpacity = fordProgress > 0 ? 1 : 0;
        slides[fordIndex].style.setProperty("--ford-gradient-edge-opacity", edgeOpacity);
        slides[fordIndex + 1].style.setProperty("--ford-gradient-edge-opacity", edgeOpacity);
      }

      // 6-7페이지 (SlideNadella -> SlideCapital) 스크롤 진행도 계산
      const nadellaIndex = slides.findIndex((s) => s.classList.contains("figma-nadella"));
      if (nadellaIndex !== -1 && slides[nadellaIndex + 1]) {
        const startTop = slides[nadellaIndex].offsetTop;
        const endTop = slides[nadellaIndex + 1].offsetTop;
        const distance = endTop - startTop;
        const nadellaProgress = distance > 0 ? Math.max(0, Math.min(1, (top - startTop) / distance)) : 0;
        
        // Slide 6 (SlideNadella)에 진행도 설정
        slides[nadellaIndex].style.setProperty("--scroll-progress", nadellaProgress.toFixed(3));
        
        // Slide 7 (SlideCapital)에도 동일한 진행도 설정
        slides[nadellaIndex + 1].style.setProperty("--scroll-progress", nadellaProgress.toFixed(3));
      }

      // 7-8페이지 (SlideCapital -> SlideRisk) 스크롤 진행도 계산
      const capitalIndex = slides.findIndex((s) => s.classList.contains("figma-capital"));
      if (capitalIndex !== -1 && slides[capitalIndex + 1]) {
        const startTop = slides[capitalIndex].offsetTop;
        const endTop = slides[capitalIndex + 1].offsetTop;
        const distance = endTop - startTop;
        const capitalProgress = distance > 0 ? Math.max(0, Math.min(1, (top - startTop) / distance)) : 0;
        
        // Slide 7 (SlideCapital)에 진행도 설정
        slides[capitalIndex].style.setProperty("--scroll-progress-capital", capitalProgress.toFixed(3));
        
        // Slide 8 (SlideRisk)에도 동일한 진행도 설정
        slides[capitalIndex + 1].style.setProperty("--scroll-progress-capital", capitalProgress.toFixed(3));
      }

      // 8-9페이지 (SlideRisk -> SlideJudgment) 스크롤 진행도 계산
      const riskIndex = slides.findIndex((s) => s.classList.contains("figma-risk"));
      if (riskIndex !== -1 && slides[riskIndex + 1]) {
        const startTop = slides[riskIndex].offsetTop;
        const endTop = slides[riskIndex + 1].offsetTop;
        const distance = endTop - startTop;
        const riskProgress = distance > 0 ? Math.max(0, Math.min(1, (top - startTop) / distance)) : 0;
        
        // Slide 8 (SlideRisk)에 진행도 설정
        slides[riskIndex].style.setProperty("--scroll-progress-risk", riskProgress.toFixed(3));
        
        // Slide 9 (SlideJudgment)에도 동일한 진행도 설정
        slides[riskIndex + 1].style.setProperty("--scroll-progress-risk", riskProgress.toFixed(3));
      }
    };
